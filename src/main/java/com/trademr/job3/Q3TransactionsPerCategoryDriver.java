...
public final class Q3TransactionsPerCategoryDriver {

  private Q3TransactionsPerCategoryDriver() {}
  ...
  public static int run(String[] args, Configuration conf) throws IOException, ClassNotFoundException, InterruptedException {
    ...
    Path out = JobPaths.outputPath(args, 1, "out/q3_per_category");
    JobPaths.deleteIfExists(conf, out);

    Job job = Job.getInstance(conf, "Q3 Transactions per category");
    MrLocal.prepareJobJar(job, Q3TransactionsPerCategoryDriver.class);

    job.setMapperClass(Q3CategoryMapper.class);
